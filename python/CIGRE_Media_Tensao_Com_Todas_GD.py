...
def load_network():
    return nw.create_cigre_network_mv(with_der="all")

def violations(net):
    pp.runpp(net)
    if net.res_line.loading_percent.max() > 95:
        return (True, "Sobrecarregamento\nde linha")
    elif net.res_trafo.loading_percent.max() > 95:
...
ax.set_ylabel("")
ax.set_xlabel("")
fig.suptitle("CIGRE - Média tensão com todos os tipos de GD", fontsize=16)
sns.despine()
plt.tight_layout()